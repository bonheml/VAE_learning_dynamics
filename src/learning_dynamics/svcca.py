...
def compute_ccas(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon,
    ...
    invsqrt_xx = linalg.sqrtm(inv_xx)
    invsqrt_yy = linalg.sqrtm(inv_yy)
    ...
    return [u.real, np.abs(s.real), v.real], invsqrt_xx.real, invsqrt_yy.real, x_idxs, y_idxs
...
def compute_pwcca(acts1, acts2, epsilon=0.):
    ...
    """
    res = get_cca_similarity(acts1, acts2, epsilon=epsilon, compute_dirns=False, compute_coefs=True, verbose=False)
    if np.sum(res["x_idxs"]) <= np.sum(res["y_idxs"]):
        idxs = res["x_idxs"]
        dirns = np.dot(res["coef_x"], (acts1[idxs] - res["neuron_means1"][idxs])) + res["neuron_means1"][idxs]
        coefs = res["cca_coef1"]
        acts = acts1
    else:
        idxs = res["y_idxs"]
        dirns = np.dot(res["coef_y"], (acts1[idxs] - res["neuron_means2"][idxs])) + res["neuron_means2"][idxs]
        coefs = res["cca_coef2"]
        acts = acts2
